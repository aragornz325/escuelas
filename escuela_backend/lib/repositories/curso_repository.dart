...
class CursoRepository extends Repository {
  Future<Map<String, dynamic>> createCurso({
    ...
    final query = await client.from('Curso').insert(nuevoCurso).execute();
  ...
  Future<List<dynamic>> createCursosBulk({
    ...
    final query = await client.from('Curso').insert(cursos).execute();
  ...
  Future<List<dynamic>> getAllCursos() async {
    final query = await client.from('Curso').select().execute();
  ...
  Future<Map<String, dynamic>> updateCurso({
    ...
    if (curso['idCurso'] != null) {
      throw Exception(
          "Error: idCurso no debe ir dentro del parametro 'curso'. Debe ir solamente en 'idCurso'.");
    }
